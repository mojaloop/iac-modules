...
output "properties_var_map" {
  value = {
    for index, msk_module in module.msk : 
      var.msk_services[index].external_resource_config.instance_address_key_name => ( msk_module.encryption_in_transit_client_broker == "PLAINTEXT" )  ? msk_module.bootstrap_brokers_plaintext : msk_module.bootstrap_brokers_tls  
  }
}
