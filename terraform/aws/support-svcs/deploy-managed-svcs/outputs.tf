output "secrets_var_map" {
  sensitive = true
  value     = ( length(local.rds_services) || length(local.mongodb_services) > 0 )> 0 ? merge(try(module.deploy_rds[0].secrets_var_map,{}), try(module.deploy_mongodb[0].secrets_var_map,{})) : {}
}
...
output "secrets_key_map" {
  value = ( length(local.rds_services) || length(local.mongodb_services) > 0 )> 0 ? merge(try(module.deploy_rds[0].secrets_key_map,{}), try(module.deploy_mongodb[0].secrets_key_map,{})) : {}
}
