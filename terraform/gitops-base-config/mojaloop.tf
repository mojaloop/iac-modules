module "generate_loki_files" {
  ...
  var_map = {
    ...
    ttksims_redis_host                          = "${var.stateful_resources[local.ttk_redis_resource_index].logical_service_name}.stateful-services.svc.cluster.local"
    ttksims_redis_port                          = var.stateful_resources[local.ttk_redis_resource_index].logical_service_port
  }
