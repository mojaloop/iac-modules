...
resource "null_resource" "run_ansible" {
  provisioner "local-exec" {
    command     = <<-EOT
          ...
          ansible-playbook mojaloop.iac.control_center_deploy -i ${local.ansible_output_dir}/inventory
    EOT
...
locals {
  jumphostmap = {
    ansible_ssh_common_args = "-o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no -o ProxyCommand=\"ssh -W %h:%p -i ${local.ansible_output_dir}/sshkey -o StrictHostKeyChecking=no -q ${var.ansible_bastion_os_username}@${var.ansible_bastion_public_ip}\""
  }
