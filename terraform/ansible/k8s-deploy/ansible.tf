...
-o UserKnownHostsFile=/dev/null -o ServerAliveInterval=60 -o StrictHostKeyChecking=no -o ProxyCommand="ssh -W %h:%p -i ${local_sensitive_file.ec2_ssh_key.filename} -o ServerAliveInterval=60 -o StrictHostKeyChecking=no -q ${var.ansible_bastion_os_username}@${var.ansible_bastion_public_ip}"
EOT
  }
