module "mojaloop_stateful_resources" {
  ...
  kv_path                                       = var.kv_path
  external_stateful_resource_instance_addresses = length(data.gitlab_project_variable.external_stateful_resource_instance_address) > 0 ? values(data.gitlab_project_variable.external_stateful_resource_instance_address)[*].value : []
}
