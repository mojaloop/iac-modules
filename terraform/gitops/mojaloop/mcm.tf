module "generate_mcm_files" {
  ...
  var_map = {
    ...
    mcm_dfsp_admin_client_secret_keycloak = join("$", ["", "{${replace(var.mcm_dfsp_admin_client_secret, "-", "_")}}"])
    dfsp_oidc_client_secret              = var.dfsp_oidc_client_secret
    dfsp_oidc_client_secret_keycloak     = join("$", ["", "{${replace(var.dfsp_oidc_client_secret, "-", "_")}}"])
    ...
    ttk_mcm_rbac_testcases_tag           = try(var.app_var_map.ttk_mcm_rbac_testcases_tag, "20.3.0-mcm-rbac.3")
    mcm_api_image_tag                    = try(var.app_var_map.mcm_api_image_tag, "v3.8.0-snapshot.19")
