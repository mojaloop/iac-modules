  ...
  content = templatefile("${local.stateful_resources_template_path}/strimzi/kafka/kafka-with-dual-role-nodes.yaml.tpl",
    {
      ...
      kafka_version          = try(each.value.local_operator_config.kafka_data.kafka_version, "4.3.0")
      kafka_metadata_version = try(each.value.local_operator_config.kafka_data.kafka_metadata_version, "4.3-IV0")
  ...
  content = templatefile("${local.stateful_resources_template_path}/dbaas/${each.value.resource_type}/db-cluster.yaml.tpl",
    {
      ...
      pitr_binlog_retention_days   = try(each.value.dbaas_resource_config.pitr_binlog_retention_days, 7)
      pitr_backup_retention_days   = try(each.value.dbaas_resource_config.pitr_backup_retention_days, 14)
...
locals {
  ...
  strimzi_kafka_grafana_dashboards_version = "1.1.0"
}
