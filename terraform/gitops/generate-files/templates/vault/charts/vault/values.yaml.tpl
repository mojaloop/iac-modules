vault:
  server:
    enabled: true
    readinessProbe:
      timeoutSeconds: ${vault_readiness_timeoutSeconds}
    dev:
      enabled: false
    extraVolumes:
      - type: configMap
    ...
    extraContainers:
      ...
      - name: init-sidecar
        ...
        imagePullPolicy: IfNotPresent
        command: ["sh","-c","cp /etc/vault/bootstrap.sh /tmp; chmod +x /tmp/bootstrap.sh; while true; do sleep 180; /tmp/bootstrap.sh; done"]
        volumeMounts:
