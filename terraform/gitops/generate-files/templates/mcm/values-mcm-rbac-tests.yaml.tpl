...
script: |
    ...
    export MCM_URL="http://mcm-connection-manager-api.${mcm_namespace}.svc.cluster.local:3001"
    export MCM_NAMESPACE="${mcm_namespace}"
    export KRATOS_PUBLIC_URL="http://kratos-public.${ory_namespace}.svc.cluster.local"
    export KRATOS_SERVICE_NAME="kratos-public.${ory_namespace}.svc.cluster.local"
    export KEYCLOAK_URL="https://${keycloak_fqdn}"
    export KEYCLOAK_FQDN="${keycloak_fqdn}"
    export KEYCLOAK_REALM="${keycloak_dfsp_realm_name}"
    ...
    export MAILPIT_NAMESPACE="${mailpit_namespace}"
