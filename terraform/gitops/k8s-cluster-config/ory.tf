module "generate_ory_files" {
  ...
  var_map = {
    ...
    keto_postgres_database               = try(module.common_stateful_resources.stateful_resources[local.keto_postgres_resource_index].logical_service_config.database_name,null)
    keto_postgres_user                   = try(module.common_stateful_resources.stateful_resources[local.keto_postgres_resource_index].logical_service_config.username,null)
    keto_postgres_host                   = "${try(module.common_stateful_resources.stateful_resources[local.keto_postgres_resource_index].logical_service_config.logical_service_name,null)}.${var.stateful_resources_namespace}.svc.cluster.local"
    keto_postgres_password_secret        = try(module.common_stateful_resources.stateful_resources[local.keto_postgres_resource_index].logical_service_config.user_password_secret,null)
    keto_postgres_port                   = try(module.common_stateful_resources.stateful_resources[local.keto_postgres_resource_index].logical_service_config.logical_service_port,null)
    keto_postgres_secret_path            = "${try(module.common_stateful_resources.stateful_resources[local.keto_postgres_resource_index].local_helm_config.generate_secret_vault_base_path,null)}/${local.keto_postgres_resource_index}/${try(module.common_stateful_resources.stateful_resources[local.keto_postgres_resource_index].local_helm_config.generate_secret_name,null)}-password"
    keto_postgres_password_secret_key    = "password"
    kratos_postgres_database             = try(module.common_stateful_resources.stateful_resources[local.kratos_postgres_resource_index].logical_service_config.database_name,null)
    kratos_postgres_user                 = try(module.common_stateful_resources.stateful_resources[local.kratos_postgres_resource_index].logical_service_config.username,null)
    kratos_postgres_host                 = "${try(module.common_stateful_resources.stateful_resources[local.kratos_postgres_resource_index].logical_service_config.logical_service_name,null)}.${var.stateful_resources_namespace}.svc.cluster.local"
    kratos_postgres_password_secret      = try(module.common_stateful_resources.stateful_resources[local.kratos_postgres_resource_index].logical_service_config.user_password_secret,null)
    kratos_postgres_port                 = try(module.common_stateful_resources.stateful_resources[local.kratos_postgres_resource_index].logical_service_config.logical_service_port,null)
    kratos_postgres_secret_path          = "${try(module.common_stateful_resources.stateful_resources[local.kratos_postgres_resource_index].local_helm_config.generate_secret_vault_base_path,null)}/${local.kratos_postgres_resource_index}/${try(module.common_stateful_resources.stateful_resources[local.kratos_postgres_resource_index].local_helm_config.generate_secret_name,null)}-password"
...
locals {
  ...
  kratos_postgres_resource_index = "kratos-db"
  keto_postgres_resource_index   = "keto-db"
