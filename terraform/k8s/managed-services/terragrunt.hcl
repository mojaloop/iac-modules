...
inputs = {
  ...
  monolith_managed_stateful_resources_config_file = find_in_parent_folders("${get_env("CONFIG_PATH")}/mojaloop-stateful-resources-ccdriven-aws-databases.yaml")
  bastion_instance_size                = local.env_vars.managed_svc_bastion_instance_size
}
