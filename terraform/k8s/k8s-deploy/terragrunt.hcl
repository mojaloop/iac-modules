...
locals {
  ...
  total_agent_count = try(sum([for node in local.enabled_node_pools : node.node_count if !node.master]), 0)
  total_master_count = try(sum([for node in local.enabled_node_pools : node.node_count if node.master]), 0)
