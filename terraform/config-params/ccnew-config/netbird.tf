...
resource "zitadel_application_oidc" "netbird_dashboard" {
  ...
  redirect_uris               = ["https://netbird.${var.tenancy_domain}/nb-auth", "https://netbird.${var.tenancy_domain}/nb-silent-auth"]
  response_types              = ["OIDC_RESPONSE_TYPE_CODE"]
  grant_types                 = ["OIDC_GRANT_TYPE_AUTHORIZATION_CODE", "OIDC_GRANT_TYPE_REFRESH_TOKEN"]
  post_logout_redirect_uris   = ["https://netbird.${var.tenancy_domain}/"]
